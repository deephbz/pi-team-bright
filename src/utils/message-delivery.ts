...
export interface DirectMessageDeliverySink {
  ...
  appendEntry(customType: typeof DIRECT_MESSAGE_ACK_ENTRY_TYPE, data: DirectMessageObservation): void;
}
  ...
  constructor(sink: DirectMessageDeliverySink, options: DirectMessageDeliveryOptions) {
    ...
    this.dependencies = {
      readUnread: options.dependencies?.readUnread
        ?? (() => messaging.readInboxForMembership(
          this.teamName,
          this.recipient,
          this.membershipId,
          true,
      ...
      markRead: options.dependencies?.markRead
        ?? ((ids) => messaging.markMessagesReadForMembership(
          this.teamName,
          this.recipient,
          this.membershipId,
          ids,
