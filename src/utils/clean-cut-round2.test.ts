...
describe.skipIf(!hasBd)("semantic task_update surface", () => {
  it("preserves task_create post-commit delivery warnings in backend and public receipts", async () => {
    ...
    expect(result.content[0].text).toContain("delivery degradation");
  ...
  it("makes expected_version optional but rejects a supplied stale token before mutation", async () => {
    ...
    const withoutToken = await tool.execute("without-token", { updates: [{ task_id: created.id, operation_id: "without-token", status: "in_progress", current_context: "v1", journal_entries: [{ kind: "progress", text: "v1" }], expected_version: created.version }] }, undefined, undefined, { sessionManager: { getSessionFile: () => `/tmp/${teamName}-lead.jsonl` } });
    const current = withoutToken.details.outcomes[0].task;
