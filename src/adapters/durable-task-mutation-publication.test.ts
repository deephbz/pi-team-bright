...
describe("Task mutation publication import fence", () => {
  it("keeps Task authority free of concrete publication imports and injects one factory", () => {
    ...
    expect(durablePort).toContain("this.taskAdapterFactory(bound.teamName, \"team-lead\")");
    expect(extension).toContain("createPublishingBeadsTaskAdapterFactory(new DurableTaskMutationPublication())");
    expect(extension).toContain("new DurableModelToolTeamPort(workerLaunchBridge, lifecycle, taskAdapterFactory, alertSender)");
