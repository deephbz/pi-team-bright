...
describe("task semantic-result totality", () => {
  it("projects every declared task_graph_apply and task_update raw outcome without generic TUI failure", () => {
    ...
    const update = [
      ...
      ...["task_not_found", "version_conflict", "operation_conflict", "invalid_transition", "legacy_transition_unsupported", "worker_mismatch", "worker_occupied", "evidence_required"].map((reason) => ({ kind: "refused", input_index: 0, task_id: task.id, operation_id: `update-${reason}`, reason, message: `${reason}.`, current_task: task, state_changed: false })),
  ...
  it("keeps every expected port outcome semantic through Pi registration", async () => {
    ...
    for (const outcome of [
      ...
      ...["task_not_found", "version_conflict", "operation_conflict", "invalid_transition", "legacy_transition_unsupported", "worker_mismatch", "worker_occupied", "evidence_required"].map((reason) => ({ kind: "refused", taskId: task.id, operationId: `update-${reason}`, reason, message: `${reason}.`, currentTask: task })),
