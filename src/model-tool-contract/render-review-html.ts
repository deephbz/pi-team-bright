...
function scenarioHtml(scenario: ModelToolCatalog["scenarios"][number], index: number): string {
  ...
  return `<article class="scenario" id="scenario-${escapeHtml(scenario.id)}">
    ...
    <div class="code-grid">
      ...
      <section><h4>Validated model projection</h4><pre class="model-return">${escapeHtml(modelContent)}</pre></section>
    </div>
    <details><summary>Raw semantic JSON</summary><pre>${json(scenario.result)}</pre></details>
...
function toolSignature(name: string): string {
  ...
  if (name === "task_graph_apply") return "task_graph_apply({ operation_id, expected_graph_version?, tasks: [{ key, title, goal, assignee, model?, needs?, on_goal_failed? }] })";
  if (name === "task_read") return "task_read({ task_ids })";
  if (name === "task_update") return "task_update({ task_id, operation_id, expected_version, transition?, current_context?, evidence? })";
...
<div class="shell">
  ...
  <main>
    ...
    <section class="projection" id="projection">
      ...
      <p>Raw semantic results remain machine truth. The validated model projection, allowlisted TUI, and QA record may omit or reorganize facts, but cannot invent state or change the semantic outcome.</p>
    </section>
