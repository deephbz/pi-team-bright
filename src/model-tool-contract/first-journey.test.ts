...
describe("first model-tool journey through the Pi registration adapter", () => {
  it("registers the exact accepted provider schemas and rejects low-level fields", async () => {
    ...
    const rejectedCalls = [
      ...
      ["task_update", { updates: [] }],
      ["task_update", { updates: [{ task_id: "task-1", operation_id: "op-1", expected_version: "task_v1", current_context: "Assigned.", journal_entries: [], team_name: "release-team" }] }],
    ] as const;
  ...
  it("rejects a malformed semantic result before model content exists", () => {
    ...
    }).toThrow("Invalid candidate team_create result.");
  ...
  it("rejects duplicate Task IDs before any update mutation", async () => {
    ...
    const duplicate = canonicalDetails(await invoke("task_update", "duplicate", {
      updates: [
        { task_id: "task-1", operation_id: "op-1", expected_version: "task_v1", current_context: "First.", journal_entries: [{ kind: "decision", text: "First." }] },
        { task_id: "task-1", operation_id: "op-2", expected_version: "task_v1", current_context: "Second.", journal_entries: [{ kind: "decision", text: "Second." }] },
      ],
    }, leaderSessionId));
  ...
  it("creates, establishes one logical Worker, and observes one Team through the exact leader Session", async () => {
    ...
    const updateConflict = canonicalDetails(await invoke("task_update", "update-conflict", {
      updates: [{ ...updateInput.updates[0], operation_id: "decision-conflict", expected_version: "task_v1" }],
    }, leaderSessionId));
