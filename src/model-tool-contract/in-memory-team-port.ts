...
export type CreateTaskPortResult =
  | { kind: "created"; task: ModelToolTaskCurrent }
...
export interface ModelToolTeamPort {
  ...
  sendAlert(leaderSessionId: ExactLeaderSessionId, input: { to: string; kind: "clarification" | "attention" | "announcement"; text: string; taskId?: string; taskVersion?: string }): Promise<AlertSendPortResult>;
  readTeamSync(
    leaderSessionId: ExactLeaderSessionId,
...
export class InMemoryModelToolTeamPort implements ModelToolTeamPort {
  ...
  async sendAlert(leaderSessionId: ExactLeaderSessionId, input: { to: string; kind: "clarification" | "attention" | "announcement"; text: string; taskId?: string; taskVersion?: string }): Promise<AlertSendPortResult> {
    const team = this.activeTeamFor(leaderSessionId);
    if (!team) return { kind: "unavailable", reason: "no_active_team", message: "The exact leader Session is not bound to an active Team." };
    if (input.to === "*" && input.kind !== "announcement") return { kind: "refused", reason: "invalid_fanout", message: "Only announcement Alerts may target the whole Team." };
    const recipients = input.to === "*" ? [...team.workersByName.keys()] : [input.to];
    const acceptedRecipients = recipients.filter((recipient) => team.workersByName.has(recipient));
    if (acceptedRecipients.length === 0) return { kind: "refused", reason: input.to === "*" ? "no_eligible_recipients" : "recipient_not_current", message: "No current recipient accepted the Alert." };
