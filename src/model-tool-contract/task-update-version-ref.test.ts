...
import { CandidateTaskUpdateParametersSchema } from "./catalog";
...
describe("candidate Task update version refs", () => {
  it("accepts one supplied change, preserves omitted context, and projects opaque refs", async () => {
    ...
    ]) expect(Check(CandidateTaskUpdateParametersSchema, { updates: [update] })).toBe(true);
    expect(Check(CandidateTaskUpdateParametersSchema, { updates: [{ task_id: "task-1", operation_id: "none", expected_version: v1 }] })).toBe(false);
    expect(Check(CandidateTaskUpdateParametersSchema, { updates: [{ task_id: "task-1", operation_id: "raw", expected_version: "task_v1", status: "open" }] })).toBe(false);
    ...
    const model = projectCandidateToolResult("task_update", raw) as { kind: string; task: { version: string } };
    expect(model.task.version).toBe(taskVersionRef("task_v2"));
