...
export class DurablePreviewTeamPort implements ModelToolTeamPort {
  ...
  async createTask(
    ...
    try {
      const task = await new CandidateBeadsTaskAdapter(bound.teamName, "team-lead").create(input);
  ...
  async sendAlert(leaderSessionId: ExactLeaderSessionId, input: { to: string; kind: "clarification" | "attention" | "announcement"; text: string; taskId?: string; taskVersion?: string }): Promise<AlertSendPortResult> {
    ...
    try {
      ...
      if (/recipient|not accepted by any current Team member/i.test(message)) {
        return { kind: "refused", reason: input.to === "*" ? "no_eligible_recipients" : "recipient_not_current", message };
      }
