...
const TaskVersion = Type.String({ minLength: 1, maxLength: 128 });
...
export const CandidateTaskCardSchema = Type.Object({
  ...
  version: TaskVersion,
}, { additionalProperties: false, description: "Current Task projection used for leader orientation." });
...
const CandidateTaskUpdateItemSchema = Type.Object({
  ...
  current_context: Type.String({ minLength: 1, maxLength: MODEL_TOOL_CANDIDATE_LIMITS.maxTaskCurrentContextChars }),
  journal_entries: Type.Array(CandidateTaskUpdateJournalEntrySchema, { minItems: 1 }),
...
export const CandidateTaskUpdateParametersSchema = Type.Object({
  ...
  additionalProperties: false,
  description: "Atomically replace current Task context and append leader journal evidence for each independent update.",
});
  ...
  Type.Object({
    ...
    outcomes: Type.Array(Type.Union([
      Type.Object({
        ...
        task: CandidateTaskCardSchema,
        journal_entries: Type.Array(CandidateTaskJournalEntrySchema, { minItems: 1 }),
      }, { additionalProperties: false }),
      Type.Object({
        ...CandidateTaskUpdateOutcomeBase,
        kind: Type.Literal("refused"),
        reason: Type.Enum(["task_not_found", "version_conflict", "operation_conflict"]),
        message: Type.String({ minLength: 1 }),
...
const TeamDelta = Type.Object({
  kind: Type.Enum(["created", "lifecycle", "purpose"]),
  text: Type.String({ minLength: 1 }),
}, { additionalProperties: false });

const WorkerDelta = Type.Object({
  worker: WorkerName,
...
export const CandidateTeamUpdatesResultSchema = Type.Object({
  ...
  team_changes: Type.Array(TeamDelta),
  worker_changes: Type.Array(WorkerDelta),
  task_changes: Type.Array(CandidateTaskDeltaSchema),
  alerts: Type.Array(AlertDelta),
}, { additionalProperties: false });
...
export const CandidateTaskLinkParametersSchema = Type.Object({
  ...
  expected_version: Type.Optional(TaskVersion),
}, {
  additionalProperties: false,
...
export const CandidateAlertSendParametersSchema = Type.Object({
  to: Type.String({ minLength: 1, maxLength: 64 }),
  kind: Type.Enum(["clarification", "attention", "announcement"]),
  text: Type.String({ minLength: 1 }),
  task_id: Type.Optional(TaskId),
  task_version: Type.Optional(TaskVersion),
}, {
  additionalProperties: false,
  description: "Send exceptional clarification, attention, or announcement. An Alert never changes Task state.",
});
...
export const candidateModelToolCatalog = {
  ...
  modelResultProjection: {
    ...
    responsibility: "Validate the semantic result and serialize unchanged named JSON for model context.",
    boundary: "Internal implementation detail only; it does not change tool semantics, extension features, or domain behavior.",
  },
  tools: [
    {
      name: "team_create",
