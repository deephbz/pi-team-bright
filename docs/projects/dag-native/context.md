...
Updated: 2026-08-14
Stage: stable-release hardening on current `origin/main`
Status: stable `0.17.0` source is selected on `release/v0.17.0` and rebased onto current `origin/main`; the complete range integrates graph-native Task authority, Worker-startup hardening, unified TUI projection, interactive graph refinement, the configurable DAG gallery, and the edge-routing follow-up; exact executable source before stable versioning passed the one allowed 1,004-test aggregate, all release-specific gates, a sequential 197-file package dry run, candidate-range privacy, and a proxy-backed real Team canary with stable Worker reuse and mechanical `A -> B` success progression; the routing follow-up then passed exact graph-path comparison, typecheck, 35 focused tests, TB and LR sentinels, gallery/export, and package verification; stable release execution must now prove exact-source E2E, aggregate and package gates, privacy classification, main CI, dry-run workflow, immutable tag, OIDC npm `latest` publication, registry integrity, provenance, and the GitHub Release
