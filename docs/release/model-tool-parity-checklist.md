...
Release candidate: `0.17.0-rc.1`
Source branch: `preview/new-model-tool-surface`

This checklist maps each published `0.16.0-rc.2` coordination capability to the
...
| `worker_ensure` | `ensure_worker({name,scope})` | Logical Worker plus existing Worker launch bridge; resource/startup/terminal composition stays in the bridge |
...
| `alert_send` | `alert_send({to,kind,text,task_id?,task_version?})` | Existing typed Alert delivery and event authority; Alerts never mutate Tasks |
...
- Result content remains minified named JSON and machine details remain the
...
- `src/model-tool-contract/first-journey.test.ts` validates registration,
  named JSON, exact binding, batch Task semantics, and replay/conflict behavior.
