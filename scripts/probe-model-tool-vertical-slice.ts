...
async function probe() {
  ...
  assert.equal(ensured.kind, "worker_ensured");
  assert.equal(ensured.effect, "created");
  assert.equal(ensured.worker.name, workerInput.name);
  assert.equal(ensured.worker.scope, workerInput.scope);
  assert.equal(ensured.worker.carrier, "absent");
  ...
  assert.equal(snapshot.workers[0]?.name, workerInput.name);
  assert.equal(snapshot.workers[0]?.scope, workerInput.scope);
  assert.equal(snapshot.workers[0]?.carrier, "absent");
  assert.deepEqual(snapshot.workers[0]?.nonterminal_task_ids, []);
