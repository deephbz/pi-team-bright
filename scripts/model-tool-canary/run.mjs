...
async function main() {
  ...
  const modelContentProjectionValid = toolResults.length === EXPECTED_TOOL_ORDER.length
    && toolResults.every((record) => {
      if (record.content?.length !== 1 || record.content[0]?.type !== "text") return false;
      try {
        const model = JSON.parse(record.content[0].text);
        return model && typeof model === "object"
          && model.kind === record.details?.kind
  ...
  const initialSnapshotAgrees = createdTask
    ...
    && initialSnapshotResult.tasks[0]?.id === taskResult.outcomes[0].task.id
    && initialSnapshotResult.tasks[0]?.status === taskResult.outcomes[0].task.status
    && initialSnapshotResult.tasks[0]?.assignee === taskResult.outcomes[0].task.assignee
    && initialSnapshotResult.tasks[0]?.version === taskResult.outcomes[0].task.version;
  const taskReadAgrees = createdTask
    ...
    && taskReadResult.outcomes[0]?.task?.id === taskResult.outcomes[0].task.id
    && taskReadResult.outcomes[0]?.task?.status === taskResult.outcomes[0].task.status
    && taskReadResult.outcomes[0]?.task?.assignee === taskResult.outcomes[0].task.assignee
    && taskReadResult.outcomes[0]?.task?.version === taskResult.outcomes[0].task.version;
  ...
  const updatesAgrees = taskUpdated
    ...
    && updatesResult.task_changes[0].journal_entries?.[0]?.id === taskUpdateResult.outcomes[0].journal_entries?.[0]?.id
    && updatesResult.task_changes[0].journal_entries?.[0]?.kind === taskUpdateResult.outcomes[0].journal_entries?.[0]?.kind
    && updatesResult.task_changes[0].journal_entries?.[0]?.text === taskUpdateResult.outcomes[0].journal_entries?.[0]?.text
    && updatesResult.task_changes[0].current?.status === taskUpdateResult.outcomes[0].task.status
    && updatesResult.task_changes[0].current?.assignee === taskUpdateResult.outcomes[0].task.assignee
    && updatesResult.task_changes[0].current?.current_context === taskUpdateResult.outcomes[0].task.current_context
    && updatesResult.task_changes[0].current?.version === taskUpdateResult.outcomes[0].task.version;
